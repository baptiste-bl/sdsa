...
{
char j[9],m[10],h[9],mer[11];
int d,a,sec;
system("date '+%A %d %B %Y %s' > ladate"	);
...
if (fic!=NULL)
	...
	while(!feof(fic))	
		{
		    fscanf(fic,"%s %d %s %d %d %s %s",j,&d,m,&a,&sec,h,mer);		
		if (!feof(fic)) 
...
void livreDispo(T_Bibliotheque *ptrB)
{
    ...
    for(int i=0;i<ptrB->nbLivres;i++)
    {
        if(strcmp(ptrB->etagere[i].emprunteur.nomemp,"") == 0)
        {
            afficherLivre(&(ptrB->etagere[i]));
            compteur = 1;
        }
...
void list_retard(T_Bibliotheque *ptrB)
{
    ...
    for(int i = 0; i < ptrB->nbLivres;i++)
    {
        if(strcmp(ptrB->etagere[i].emprunteur.nomemp,"") != 0)
        {
            if( Nbsec < (E.seconde - ptrB->etagere[i].emprunteur.seconde))
            {
                test = 1;
                printf("\n RETARD: \n");
                afficherLivre(&(ptrB->etagere[i]));
